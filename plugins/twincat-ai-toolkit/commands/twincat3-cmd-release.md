...
3. Export with **`twincat_export_library(library=true, compiled_library=false)`** — only `.library` (faster). Add `.compiled-library` only if the user explicitly asks.
4. Confirm version with the user if unclear.
