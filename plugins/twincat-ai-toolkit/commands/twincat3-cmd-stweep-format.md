---
name: twincat3-cmd-stweep-format
description: >-
...
4. Format **file or folder** with `twincat_stweep_format(path=...)`.
5. Whole **project** only if the user explicitly confirms → `confirm=true`.
6. Default `wait=false`; raise `timeout_seconds`; poll `twincat_stweep_format_progress` until `running=false`.
...
8. To abort: `twincat_stweep_format_cancel` (stops between files).
