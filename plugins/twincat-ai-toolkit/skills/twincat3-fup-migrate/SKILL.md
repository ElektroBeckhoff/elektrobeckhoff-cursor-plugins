...
description: >-
  ...
  backup/swap/replace modes. Use when converting FBD/FUP to text-based
...
**Safe output (recommended first time):**
```
twincat_fup_migrate(input="<path>", swap=false)
```
...
**In-place replace (only if user explicitly requests):**
```
twincat_fup_migrate(input="<path>", replace=true)
```
...
| Safe generation | `swap=false` |
| Production swap | (default, no extra flags) |
| Folder recursive | `recursive=true` |
| In-place replace | `replace=true` (requires confirmation) |
